...
public class ConstructionGrid : MonoBehaviour
{
    ...
    public void CellSpawn()
    {
        ...
        GameObject[] allObject = new GameObject[transform.childCount];
        
        for (int i = 0; i < transform.childCount; i++)
        {
            allObject[i] = transform.GetChild(i).gameObject;
        }
        for (int i = 0; i < allObject.Length; i++)
        {
            if (allObject[i].GetComponent<Cell>() == null) continue;
    ...
    public Vector3 ConvertWorldPositionToCellWorldPosition(Vector3 mousePosition)
    {    
    ...
    public Vector2 ConvertWorldPositionToLocalCellPosition(Vector3 mousePosition)
    {
